            ...
            <div class="container px-4 sm:px-0 mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-16 py-32">
                <div class="pl-8 col-span-1 lg:col-span-3 flex flex-col md:flex-row flex-wrap gap-2 justify-center content-start md:justify-start md:content-center">
                  ...
                  <ul class="flex flex-row flex-none flex-wrap gap-1 justify-start content-center">
                    <li v-for="tag in project.researchtags" :key="tag" class="flex-none max-h-6 font-main text-grayMiddle border-grayMiddle border text-xs px-2.5 py-0.5">{{ tag }}</li>
                    <li v-for="tag in project.designtags" :key="tag" class="flex-none max-h-6 font-main text-grayMiddle border-grayMiddle border text-xs px-2.5 py-0.5">{{ tag }}</li>
                    <li v-for="tag in project.impacttags" :key="tag" class="flex-none max-h-6 font-main text-grayMiddle border-grayMiddle border text-xs px-2.5 py-0.5">{{ tag }}</li>
                  </ul>
                </div>
